...
class PipecatEngine:
    ...
    async def _generate_call_summary(self) -> str:
        ...
        return (self._gathered_context or {}).get("call_disposition") or ""
    ...
    async def _emit_session_outputs(self, reason: str) -> None:
        ...
        serializer = getattr(
            getattr(transport_output, "_params", None), "serializer", None
        )
        if serializer is None or not getattr(
            serializer, "supports_session_outputs", False
        ):
            return
        ...
        try:
            ...
            tool_reason = gathered.get("call_disposition")
            if reason == EndTaskReason.END_CALL_TOOL_REASON.value and tool_reason:
            ...
            logger.debug(
                f"[run {self._workflow_run_id}] Session outputs queued: {outputs}"
            )
        except Exception as e:
